...
- 把讲义转成 **Anki 词表 CSV**
- **商品展示/售卖文案**：写 `<项目>/marketing/卖点文案_<文章名>.md`。结构固定 8 节：主标题钩子 → 详情页卖点（每条必须带**可验证数字**）→ 规格参数表 → 适合谁 → 说明 → 短文案 4 版（痛点/人群/一句话/首图大字）→ **发布前合规自查** → 配图素材。
  - 最有说服力的卖点是**真题例句可溯源**（条数 + 覆盖卷别数 + 年份跨度），这是与网上编造句例的硬差异。
  - **数字必须先 grep 核准**：`grep -cE "^[a-z][a-z' -]* \| /" src/x.md`（词数）、`grep -cE "\| own \|"`（自编例句数）、`grep -oE "\| 20[0-9]{2} 英语[一二] \|" | sort -u | wc -l`（覆盖卷别数）。**凭印象报数会出错**。
  - 合规红线（必须写进文案）：不附赠真题 PDF／原文版权归 The Economist／禁用"绝密·押题·保分"／虚拟资料类目有资质门槛／按"单篇讲义"定价而非"题库"。
